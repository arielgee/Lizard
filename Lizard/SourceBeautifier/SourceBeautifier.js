...
/////////////////////////////////////////////////////////////////////////////////////////////
///
let SourceBeautifier = (function () {

	const HTML_DEF_OPTIONS = {
		indent_size: 3,
		indent_character: " ",
	...
	const CSS_DEF_OPTIONS = {
		indent_size: 3,
		indent_character: " ",
	};

	const JS_DEF_OPTIONS = {
		indent_size: 3,
		indent_character: " ",
		...
		brace_style: "end-expand",	// "collapse" | "expand" | "end-expand" | "expand-strict"
	};

	//////////////////////////////////////////////////////////////////////
	function html(html_source, options) {
		...
		options = options || {};
		indent_size = options.indent_size || HTML_DEF_OPTIONS.indent_size;
		indent_character = options.indent_char || HTML_DEF_OPTIONS.indent_character;
		brace_style = options.brace_style || HTML_DEF_OPTIONS.brace_style;
		max_char = options.max_char == 0 ? Infinity : options.max_char || HTML_DEF_OPTIONS.max_char;
		unformatted = options.unformatted || HTML_DEF_OPTIONS.unformatted;
		...
		function Parser() {
			...
			this.get_contents_to = function (name) { //get the full content of a script or style to pass to js_beautify
				...
				let input_char = '';
				let content = '';
				let reg_match = new RegExp('\<\/' + name + '\\s*\>', 'igm');
				reg_match.lastIndex = this.pos;
				let reg_array = reg_match.exec(this.input);
				let end_script = reg_array ? reg_array.index : this.input.length; //absolute end of script
			...
			this.get_tag = function () { //function to get a full tag and parse its type
				...
				if (tag_complete.charAt(tag_complete.length - 2) === '/' ||
					...
					let comment = this.get_unformatted('</' + tag_check + '>', tag_complete); //...delegate to get_unformatted function
					...
					if (tag_check.indexOf('[if') != -1) { //peek for <!--[if conditional comment
						if (tag_complete.indexOf('!IE') != -1) { //this type needs a closing --> so...
							let comment = this.get_unformatted('-->', tag_complete); //...delegate to get_unformatted
							content.push(comment);
						}
						this.tag_type = 'START';
					}
						...
						let comment = this.get_unformatted(']]>', tag_complete); //...delegate to get_unformatted function
						content.push(comment);
			...
			this.get_token = function () { //initial handler for token-retrieval
				...
				if (this.last_token === 'TK_TAG_SCRIPT' || this.last_token === 'TK_TAG_STYLE') { //check if we need to format javascript
					let type = this.last_token.substr(7)
			...
			switch (multi_parser.token_type) {
				...
				case 'TK_TAG_END':
					...
					if (multi_parser.last_token === 'TK_CONTENT' && multi_parser.last_text === '') {
						let tag_name = multi_parser.token_text.match(/\w+/)[0];
						let tag_extracted_from_last_output = multi_parser.output[multi_parser.output.length - 1].match(/<\s*(\w+)/);
				...
				case 'TK_TAG_SINGLE':
					...
					let tag_check = multi_parser.token_text.match(/^\s*<([a-z]+)/i);
				...
				case 'TK_SCRIPT':
					if (multi_parser.token_text !== '') {
						...
						let text = multi_parser.token_text, _beautifier, script_indent_level;
						...
						let indentation = multi_parser.get_full_indent(script_indent_level);
						if (_beautifier) {
							// call the Beautifier if avaliable
							text = _beautifier(text.replace(/^\s*/, indentation), options);
						} else {
							// simply indent the string otherwise
							let white = text.match(/^\s*/)[0];
	...
	//////////////////////////////////////////////////////////////////////
	function css(css_source, options) {
		options = options || {};
		let indentSize = options.indent_size || CSS_DEF_OPTIONS.indent_size;
		let indentCharacter = options.indent_char || CSS_DEF_OPTIONS.indent_character;
		...
		let pos = -1, ch;
		function next() {
			return ch = css_source.charAt(++pos)
		}
		function peek() {
			return css_source.charAt(pos + 1)
		}
		function eatString(comma) {
			...
			return css_source.substring(start, pos + 1);
		}
		...
		// printer
		let indentString = css_source.match(/^[\r\n]*[\t ]*/)[0];
	...
	//////////////////////////////////////////////////////////////////////
	function js(js_source, options) {

		let input, output, token_text, last_type, last_text, last_last_text, last_word, flags, flag_store, indent_string;
		let whitespace, wordchar, punct, parser_pos, line_starters, digits;
		let prefix, token_type, do_block_just_closed;
		let wanted_newline, just_added_newline, n_newlines;
		...
		opt_brace_style = options.brace_style ? options.brace_style : (opt_brace_style ? opt_brace_style : JS_DEF_OPTIONS.brace_style);


		let opt_indent_size = options.indent_size ? options.indent_size : JS_DEF_OPTIONS.indent_size,
			opt_indent_char = options.indent_char ? options.indent_char : JS_DEF_OPTIONS.indent_character,
			opt_preserve_newlines = typeof options.preserve_newlines === 'undefined' ? JS_DEF_OPTIONS.preserve_newlines : options.preserve_newlines,
			opt_break_chained_methods = typeof options.break_chained_methods === 'undefined' ? JS_DEF_OPTIONS.break_chained_methods : options.break_chained_methods,
			opt_max_preserve_newlines = typeof options.max_preserve_newlines === 'undefined' ? JS_DEF_OPTIONS.max_preserve_newlines : options.max_preserve_newlines,
			opt_jslint_happy = options.jslint_happy === 'undefined' ? JS_DEF_OPTIONS.jslint_happy : options.jslint_happy,
			opt_keep_array_indentation = typeof options.keep_array_indentation === 'undefined' ? JS_DEF_OPTIONS.keep_array_indentation : options.keep_array_indentation,
			opt_space_before_conditional = typeof options.space_before_conditional === 'undefined' ? JS_DEF_OPTIONS.space_before_conditional : options.space_before_conditional,
			opt_unescape_strings = typeof options.unescape_strings === 'undefined' ? JS_DEF_OPTIONS.unescape_strings : options.unescape_strings;

		just_added_newline = false;

		// cache the source's length.
		let input_length = js_source.length;
		...
		function print_newline(ignore_repeated, reset_statement_flags) {
			...
			for (let i = 0; i < flags.indentation_level + flags.chain_extra_indentation; i += 1) {
		...
		function get_next_token() {
			...
			if (in_array(c, wordchar)) {
				...
				if (wanted_newline && last_type !== 'TK_OPERATOR'
					&& last_type !== 'TK_EQUALS'
					&& !flags.if_line && (opt_preserve_newlines || last_text !== 'let')) {
					print_newline();
				}
				return [c, 'TK_WORD'];
			}
		...
		while (js_source && (js_source.charAt(0) === ' ' || js_source.charAt(0) === '\t')) {
			preindent_string += js_source.charAt(0);
			js_source = js_source.substring(1);
		}
		input = js_source;
		...
		line_starters = 'continue,try,throw,return,let,if,switch,case,default,for,while,break,function'.split(',');
		...
		while (true) {
			...
			switch (token_type) {
				...
				case 'TK_WORD':
					...
					if (in_array(token_text.toLowerCase(), ['else', 'catch', 'finally'])) {
						...
						if (is_special_word(last_text)) {
							...
							if ((last_type !== 'TK_START_EXPR' || token_text !== 'let') && last_text !== ':') {
				...
				case 'TK_EQUALS':
					if (flags.var_line) {
						// just got an '=' in a let-line, different formatting/line-breaking, etc will now be done
				...
				case 'TK_COMMA':
					if (flags.var_line) {
						if (is_expression(flags.mode) || last_type === 'TK_END_BLOCK') {
							// do not break on comma, for(let a = 1, b = 2)
							flags.var_line_tainted = false;
						}
		...
		let sweet_code = preindent_string + output.join('').replace(/[\r\n ]+$/, '');
		return sweet_code;

...
class LizardDB {
	...
	updateRulesByUrl(url, details = {}) {
		...
		return new Promise((resolve, reject) => {
			...
			let tran = this._getRulesTransaction("readwrite", (error) => {
				console.log("[Lizard]", "updateRulesByUrl transaction error/abort", error.name, error.message);
				return reject(error);
			});
			...
			reqCursor.onsuccess = () => {
				...
				if(cursor) {
					...
					const obj = Object.assign(this._getDefaultRuleObject(url), cursor.value, details);

					if(typeof(details.url) === "string") obj.url = details.url;
					if(typeof(details.cssSelector) === "string") obj.cssSelector = details.cssSelector;
					if(this._isBoolean(details.hide)) obj.hide = details.hide;
					if(this._isBoolean(details.remove)) obj.remove = details.remove;
					if(this._isBoolean(details.dewidthify)) obj.dewidthify = details.dewidthify;
					if(this._isBoolean(details.isolate)) obj.isolate = details.isolate;
					...
					reqPut.onerror = (event) => {
						const error = event.target.error;
						console.log("[Lizard]", "updateRulesByUrl put error",error.name, error.message);
						reject(error);
					};
			...
			reqCursor.onerror = (event) => {
				const error = event.target.error;
				console.log("[Lizard]", "updateRulesByUrl delete error",error.name, error.message);
				reject(error);
			};
		});
	}
